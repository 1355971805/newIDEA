...
layui.define(['element', 'layer', 'form'], function (exports) {
    ...
    form.on('submit(login)', function (data) {
        ...
        setTimeout(function () {
            //模拟登陆
            layer.close(index);
           var uname= $('[name=userCode]').val();
            var upwd= $('[name=userPwd]').val();
             location.href="/UserInfoServlet?userCode="+uname+"&userPwd="+upwd;
        }, 400);
        return false;
    });
    //检测键盘按下
    ...
    function login() {
        ...
        loginHtml += '<input type="text" name="userCode" lay-verify="account" placeholder="请输入账号" value="lyblogscn" autocomplete="off" class="layui-input">';
        loginHtml += '</div>';
        loginHtml += '</div>';
        loginHtml += '<div class="layui-form-item">';
        loginHtml += '<label class="layui-form-label">密码</label>';
        loginHtml += '<div class="layui-input-inline pm-login-input">';
        loginHtml += '<input type="password" name="userPwd" lay-verify="passWord" placeholder="请输入密码" value="111111" autocomplete="off" class="layui-input">';
        loginHtml += '</div>';
        loginHtml += '</div>';
        loginHtml += '<div class="layui-form-item">';
